...
% --- Document Info ---
\title{Chapter 1: Linear Regression \& Optimization}
...
\setcounter{chapter}{1} % Setting to Chapter 1
\chapter*{Chapter 1: Linear Regression \& Optimization}
\addcontentsline{toc}{chapter}{Chapter 1: Linear Regression \& Optimization}

\section{Hypothesis Representation and the Cost Function}

To perform supervised learning, we must decide how we are going to represent our hypothesis function $h$ in a computer. As an initial choice, we approximate $y$ as a linear function of $x$:
...
Where:
\begin{itemize}
    \item \textbf{$d$} = the total number of features in the dataset.
    \item \textbf{$x$} = the input feature vector, represented by a column matrix of order $(d+1) \times 1$ (assuming an intercept term $x_0 = 1$).
    \item \textbf{$\theta$} = the parameter vector, represented by a column matrix of order $(d+1) \times 1$.
\end{itemize}

Given a training set, our goal is to pick, or learn, the parameters $\theta$ such that $h(x)$ is as close to $y$ as possible for our training examples. We quantify this \textbf{total error} across the entire training set using the \textbf{Cost Function} (also called the Objective Function or Loss Function):
...
\section{The Geometry of Learning Spaces}
...
and the corresponding design matrix is $X \in \mathbb{R}^{n \times (d+1)}$. \textit{(Note: See Appendix A.1 regarding the distinction between input space and feature space).}

The \textbf{Parameter Space ($\Theta$)} is a coordinate space whose axes correspond to the model parameters (or weights). Each point in this space represents a unique configuration of the model. During training, the parameter vector moves through the parameter space as the optimization algorithm searches for values that minimize the loss function.
...
\toprule
\textbf{Property} & \textbf{Feature Space (Data World)} & \textbf{Parameter Space (Model World)} \\
\midrule
\textbf{Concept} & Physical input features of the samples. & Parameters/weights of the model. \\
\textbf{Axes} & One specific feature (e.g., $x_1 = \text{Sq. Ft.}$). & One specific weight (e.g., $\theta_1 = \text{Price/Sq. Ft.}$). \\
\textbf{Points} & A single training example (one house). & A fully configured model hypothesis. \\
\textbf{Dimensions} & $d+1$ dimensions (including $x_0=1$). & $d+1$ dimensions (every feature gets a weight). \\
\textbf{Training} \newline \textbf{Dynamics} & Static. Data points act as fixed anchors. & Dynamic. The vector $\theta$ moves to minimize loss. \\
\bottomrule
\end{tabular}
\end{table}

\subsection{The Loss Hypersurface}
...
\begin{figure}[H]
    \centering
    \begin{tcolorbox}[colback=gray!10, colframe=gray!50, width=0.8\textwidth, center]
        \centering
        \textit{[Image Placeholder: A 3D convex bowl shape representing the squared error cost function plotted over a 2D parameter space, showing a global minimum at the bottom.]}
    \end{tcolorbox}
    \caption{Squared Error Loss Hypersurface}
\end{figure}

Geometrically, this creates a \textbf{Hypersurface} in $\mathbb{R}^{d+2}$ space. If the parameter space is a 2D floor, the loss function creates a 3D mountain range or bowl hovering above it, where altitude represents the error. 

\subsection{The Gradient Space (The Vector Field)}

The gradient operator $\nabla_\theta J(\theta)$ maps a point in the parameter space to a vector that exists \textit{entirely within the same parameter space}: $\nabla_\theta J(\theta): \mathbb{R}^{d+1} \to \mathbb{R}^{d+1}$. The gradient does NOT point up into the 3D bowl (the $(d+2)$-dimensional space of the hypersurface). It is a vector living entirely on the $(d+1)$ floor, pointing toward the coordinates of the steepest uphill path.

\section{The Least Mean Squares (LMS) \& Gradient Descent Algorithm}
...
\begin{itemize}
    \item \textbf{$\theta_j$}: The individual parameter (weight). $\theta_0$ is the bias (y-intercept).
    \item \textbf{$\alpha$ (Learning Rate)}: A positive scalar tuning parameter dictating step size. If $\alpha$ is too small, convergence is slow. If $\alpha$ is too large, the algorithm can overshoot the minimum or diverge.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Batch Gradient Descent:} Uses the entire dataset to calculate the gradient for a single step. Highly accurate but computationally expensive for massive datasets.
    \item \textbf{Stochastic Gradient Descent (SGD):} Uses only one random data point to calculate the gradient and update weights. Incredibly fast, but the path to the minimum is noisy and zig-zags wildly.
    \item \textbf{Mini-Batch Gradient Descent:} The industry standard. Splits data into small groups (e.g., 32, 64, or 128 samples), balancing the speed of SGD with the stability of Batch Gradient Descent.
\end{enumerate}

\section{The Mechanics of Gradient Descent}

The gradient vector follows two rules:
\begin{enumerate}
    \item \textbf{It Points to the Steepest Ascent:} It points in the direction of the steepest increase of the loss function. Therefore, moving in the opposite direction, $-\nabla_\theta J (\theta)$, yields the direction of steepest descent.
    \item \textbf{Its Length is the Slope:} The magnitude dictates steepness. Far from the minimum, the vector is large (bigger steps). Near the bottom, derivatives shrink toward zero, shortening the vector and preventing overshooting.
\end{enumerate}
...
\begin{tcolorbox}[title=Interactive Widget Placeholder: Gradient Descent 3D Visualizer, colback=blue!5, colframe=blue!50!black]
\textbf{Objective:} Create an interactive 3D surface plot of a convex bowl (squared error cost function) to visualize gradient descent.\\
\textbf{Inputs:}
\begin{itemize}
    \item Learning Rate (Slider)
    \item Starting X coordinate (Slider)
    \item Starting Y coordinate (Slider)
    \item Step/Run Button
\end{itemize}
\textbf{Behavior:} Render a 3D convex bowl surface ($z = x^2 + y^2$). Show a point representing the current parameters. When the user clicks step/run, animate the point descending the gradient towards the global minimum (0,0,0) based on the learning rate.
\end{tcolorbox}
...
Let $v \in \mathbb{R}^{d+1}$ be an arbitrary unit vector ($\|v\| = 1$). The \textbf{Directional Derivative} of $J$ at point $\theta$ in the direction of $v$, denoted $D_v J(\theta)$, measures the instantaneous rate of change of the cost function as we move along $v$. By the chain rule:
...
\begin{itemize}
    \item \textbf{Steepest Ascent:} Maximized when $\cos(\phi) = 1$ ($\phi = 0$). $v$ points in the \textit{exact same} direction as the gradient.
    \item \textbf{Steepest Descent:} Minimized when $\cos(\phi) = -1$ ($\phi = \pi$). $v$ points in the \textit{exact opposite} direction of the gradient.
\end{itemize}
...
\subsection{The Single Training Example (Widrow-Hoff Rule)}
...
\begin{quote}
\textbf{Intuition:} The term $(y^{(i)} - h_\theta(x^{(i)}))$ represents pure error. Under-predictions (positive error) increase weight $\theta_j$, while over-predictions (negative error) decrease it. The update magnitude is proportional to the error and the specific feature magnitude $x_j^{(i)}$.
\end{quote}
...
\begin{quote}
\textbf{Note 1:} Batch gradient descent has to scan through the entire training set before taking a single step, which is computationally prohibitive for large datasets. SGD, conversely, begins optimizing parameters immediately. \\
\textbf{Note 2:} SGD moves $\theta$ "close" to the minimum much faster than batch gradient descent, but it may never truly "converge." The parameters will continue oscillating around the global minimum of $J(\theta)$, though in practice, these values are reasonably good approximations to the true minimum.
\end{quote}

\section{The Normal Equations (Closed-Form Solution)}
...
\textbf{2. Geometrically: A Single Location in $\mathbb{R}^{n \times d}$} \\
...
\textbf{3. The Mathematical Relationship} \\
...
\subsection{The Least Squares Revisited}

We group our training inputs into a design matrix $X$ and our targets into a vector $\vec{y}$:

\[
X = \begin{bmatrix} \text{\textemdash} & (x^{(1)})^T & \text{\textemdash} \\ \text{\textemdash} & (x^{(2)})^T & \text{\textemdash} \\ & \vdots & \\ \text{\textemdash} & (x^{(n)})^T & \text{\textemdash} \end{bmatrix}_{n \times (d+1)} , \quad \vec{y} = \begin{bmatrix} y^{(1)} \\ y^{(2)} \\ \vdots \\ y^{(n)} \end{bmatrix}_{n \times 1}
\]
...
\section{Probabilistic Interpretation for Least-Squares}
...
\textit{(Note: We use a semicolon in $p(y^{(i)} \mid x^{(i)}; \theta)$ because $\theta$ is a fixed, deterministic parameter, not a random variable).}

\subsection{Maximizing Log-Likelihood ($\ell(\theta)$)}
...
\subsection{Connection to Least-Squares}
...
This proves that under the assumption of IID Gaussian noise, \textbf{Ordinary Least Squares (OLS) regression is structurally identical to Maximum Likelihood Estimation.}

% --- Appendix ---
\appendix
\chapter{Appendices}

\section{Difference Between Input Space $\mathcal{X}$ and Feature Space $\mathcal{F}$}
...
\begin{itemize}
    \item \textbf{Input Space ($\mathcal{X}$):} The set of raw inputs provided to the model. Mathematically, $\mathcal{X} \subseteq \mathbb{R}^d$.
    \item \textbf{Feature Space ($\mathcal{F}$):} The set of representation vectors the model actually operates on, obtained via $\phi(x) \in \mathcal{F}$.
\end{itemize}

\textbf{Relationship:}
\begin{itemize}
    \item If no transformation is applied ($\phi(x) = x$), then $\mathcal{F} = \mathcal{X}$.
    \item If feature engineering is applied ($\phi(x) \neq x$), then $\mathcal{F} \neq \mathcal{X}$.
\end{itemize}
...
\section{Notation Nuances of the Design Matrix}
